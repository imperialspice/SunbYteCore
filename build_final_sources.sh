...
old_dir=$(pwd)
root_dir=$(pwd)/deps
install_path=/home/build/odroid_target
CROSS_TARGET="-DCMAKE_TOOLCHAIN_FILE=./aarch64_toolchain.cross")
...
cmake -S $root_dir/downloads/opencv -B $root_dir/sources/opencv $CROSS_TARGET "${OPENCV_CMAKE_OPTIONS[@]}" -DCMAKE_INSTALL_PREFIX="$install_path";
# goto source dir
...
cp -r $root_dir/downloads/cereal/include $install_path/include
...
"$root_dir"/downloads/libgpiod/autogen.sh --host=aarch64-linux-gnu  --enable-bindings-cxx  --prefix="$install_path"
make -j $noCores
make install
...
cmake -S $root_dir/downloads/spidev-lib-master -B $root_dir/sources/spidev $CROSS_TARGET "${OPENCV_CMAKE_OPTIONS[@]}" -DCMAKE_INSTALL_PREFIX="$install_path";
## goto source dir
...
OPENCV_CMAKE_OPTIONS=();
cmake -S $root_dir/downloads/libevent -B $root_dir/sources/libevent $CROSS_TARGET "${OPENCV_CMAKE_OPTIONS[@]}" -DCMAKE_INSTALL_PREFIX="$install_path";
